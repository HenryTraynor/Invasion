...
coexistence_list <- replicate(numRealizations,
                              alphaSim(att.param, singleWindow.time.param, ratio.max=1, ttb=25, do.win=FALSE),
                              simplify=F)
...
for(i in 1:numRealizations) {
  ...
  endemicPlot = endemicPlot + geom_line(data=endemic_list[[i]], aes(x=time, y=invader), color="red", alpha=0.2) + geom_line(data=endemic_list[[i]], aes(x=time, y=endemic), color="blue", alpha=0.2)
  
  coexistencePlot = coexistencePlot + geom_line(data=coexistence_list[[i]], aes(x=time, y=invader), color="red", alpha=0.2) + geom_line(data=coexistence_list[[i]], aes(x=time, y=endemic), color="blue", alpha=0.2)
  
  invaderPlot = invaderPlot + geom_line(data=invader_list[[i]], aes(x=time, y=invader), color="red", alpha=0.2) + geom_line(data=invader_list[[i]], aes(x=time, y=endemic), color="blue", alpha=0.2)
}
# add alpha=1.0 curves
endemicPlot = endemicPlot + geom_line(data=endemic_list[[1]], aes(x=time, y=invader), color="red") + geom_line(data=endemic_list[[1]], aes(x=time, y=endemic), color="blue")
coexistencePlot = coexistencePlot + geom_line(data=coexistence_list[[1]], aes(x=time, y=invader), color="red") + geom_line(data=coexistence_list[[1]], aes(x=time, y=endemic), color="blue")
invaderPlot = invaderPlot + geom_line(data=invader_list[[1]], aes(x=time, y=invader), color="red") + geom_line(data=invader_list[[1]], aes(x=time, y=endemic), color="blue")


grid.arrange(textGrob("Species Abundance for Three Outcomes", gp=gpar(size=32)), endemicPlot+ylab("Individuals"), coexistencePlot+theme(axis.title.y=element_blank()), invaderPlot+theme(axis.title.y=element_blank()), 
             ncol=3, nrow = 2, 
...
plot = plot +
  geom_line(data=inc_list[[1]], aes(x=time, y=invader, color="red")) +
  geom_line(data=const_list[[1]], aes(x=time, y=invade, color="blue"))
