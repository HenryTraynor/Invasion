...
setOfIncAlphaSimPlot <- function(numRealizations, att.param, singleWindow.time.param, do.prob, ttb) {
  time.max = 50
  tau = singleWindow.time.param$tau
  df.data <- data.frame(time=seq(0,time.max+tau, by=tau))
  i=0
  while(i<numRealizations) {
    df.subData1 = alphaSim(att.param, singleWindow.time.param, do.prob=TRUE, ratio.max=2, ttb=25, do.win=TRUE)
    df.subData1 = cbind(df.subData1, data.frame(index=replicate(length(seq(0,time.max+tau, by=tau)), paste(i))))
    colnames(df.subData1) = c("time", paste("endemic",i+1,sep=""), paste("invaderInc",i+1,sep=""), "ratio", "index")
    
    df.subData2 = alphaSim(att.param, singleWindow.time.param, do.prob=TRUE, ratio.max=0.5, ttb=25, do.win=FALSE)
    df.subData2 = cbind(df.subData2, data.frame(index=replicate(length(seq(0,time.max+tau, by=tau)), paste(i))))
    colnames(df.subData2) = c("time", paste("endemic",i+1,sep=""), paste("invaderConstant",i+1,sep=""), "ratio", "index")
    
    df.data = cbind(df.data, df.subData1[3], df.subData2[3])
    i=i+1
  }
  plot = ggplot(data=df.data, aes(x=time)) + geom_line(aes(y=df.data[,2]), color="red") +  geom_line(aes(y=df.data[,3]), color="blue") + xlab("Time (Years)")
  #plot = plot + geom_line(aes(y=invaderInc1, color="red")) + geom_line(aes(y=invaderConstant1, color="blue"))
  seqj = seq(4,numRealizations*2+1, by=2)
  for(j in seqj) {
    plot = plot + geom_line(aes(x=time,y=df.data[,j]), color="red", alpha=0.2) + geom_line(aes(x=time,y=df.data[,j+1]), color="blue", alpha=0.2)
  }
  plot = plot + theme(legend.position="none") + ylab("Abundance")
  return(plot)
}

time.max = 50
tau = singleWindow.time.param$tau
df.dataI <- data.frame(time=seq(0,time.max+tau, by=tau), index=replicate(numRealizations*(time.max/tau+2), NA))

columnsI = c("time", "index", "invader")

df.dataI <- data.frame(matrix(nrow =0, ncol = length(columnsI)))
colnames(df.dataI) <- columnsI

for(i in numRealizations) {
  tempI = alphaSim(att.param, singleWindow.time.param, do.prob=TRUE, ratio.max=2, ttb=25, do.win=TRUE)
  df.dataI = rbind(df.dataI, cbind(select(tempI, time, invader),replicate(length(tempI[,1]), i)))
}


setOfIncAlphaSimPlot(10, att.param, singleWindow.time.param, do.prob=TRUE, ttb=50)
```
