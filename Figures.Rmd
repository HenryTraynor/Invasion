...
ggplot(data=df.realizations, aes(x=time1)) + geom_line(aes(y=invader1), cex=1.5, color="red") + geom_line(aes(y=invader2), cex=1.5, color="blue") + xlab("Time (years)") + ylab("Invader Abundance") + ggtitle("Invader abundance for two types of realizations") + guides(color= guide_legend(title = "Realization")) + theme(legend.position="bottom")
```
...
time.parameters = as.data.frame(t(c('1 Week', '10 Years', '1 Year', '100 each')))
...
```{r ROC Curve}
library(DescTools)
df.rates <- ROCcurveData(df.SD, numThresholds = 200, inequality = ">")
auc <- AUC(x=df.rates$FPR, y=df.rates$TPR, method='trapezoid')
ggplot(data=df.rates, aes(x=FPR, y=TPR)) + geom_point(size=4, color='#549E39') + geom_abline(slope=1, intercept=0, linetype=3, cex=2) + ggtitle("TPR vs. FPR for SD as a diagnostic") + theme(legend.position="bottom")
```
...
library(moments)
# increasing ratio
df.increasing <- halfAndHalf(att.param, singleWindow.time.param, ttb=25, halfSimulations=75, win.ratio=2, fail.ratio=0.1, increasing=TRUE)
# constant ratio: a12 > a21
df.constant <- halfAndHalf(att.param, singleWindow.time.param, ttb=25, halfSimulations=75, win.ratio=2, fail.ratio=0.1, increasing=FALSE)
