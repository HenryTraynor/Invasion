...
df.constant <- halfAndHalf(att.param, singleWindow.time.param, ttb=25, halfSimulations=50, win.ratio=2, fail.ratio=0.1, increasing=TRUE)
...
dfListing <- function(df.increasing, df.constant) {
  # mean
  df.mean.increasing = singleStatWindows(df.increasing, windows, time.index, 1)
  df.mean.constant = singleStatWindows(df.constant, windows, time.index, 1)
  # sd
  df.sd.increasing = singleStatWindows(df.increasing, windows, time.index, 2)
  df.sd.constant = singleStatWindows(df.constant, windows, time.index, 2)
  # cv
  df.cv.increasing = singleStatWindows(df.increasing, windows, time.index, 3)
  df.cv.constant = singleStatWindows(df.constant, windows, time.index, 3)
  # kurtosis
  df.kurtosis.increasing = singleStatWindows(df.increasing, windows, time.index, 4)
  df.kurtosis.constant = singleStatWindows(df.constant, windows, time.index, 4)
  # skewness
  df.skewness.increasing = singleStatWindows(df.increasing, windows, time.index, 5)
  df.skewness.constant = singleStatWindows(df.constant, windows, time.index, 5)
...
ROCdataList <- function(statsList, thresholds) {
  #increasing
...
ROCcurve.sd.increasing <- ggplot() +
  geom_point(data=ROCdata[[1]], aes(x=FPR1, y=TPR1, color="1 year")) +
  geom_point(data=ROCdata[[1]], aes(x=FPR2, y=TPR2, color="2 years")) +
  geom_point(data=ROCdata[[1]], aes(x=FPR3, y=TPR3, color="3 years")) +
